		...
		foreach ($steps as $step) {
			...
			if ($step == $protokd_result_none) {
				if (sizeof($results) == 1) {
					...
					$route_output[] = array("none", "none", array($start, $finish), $message_routenotfound[$presentation], null);
					$travel_time = null;
					break;
				} else {
			...
			if ($means == $protokd_transitmode_walk) {
				...
				$result = mysqli_query($global_mysqli_link, "SELECT tracks.trackname, tracktypes.name, tracktypes.url, tracks.extraParameters, tracktypes.speed FROM tracks JOIN tracktypes ON tracktypes.trackTypeId='$means' AND tracks.trackTypeId='$means' AND tracks.trackid='$means_detail'") or
...
// 		$full_url = "$places_url?client_id=$foursq_client_id&client_secret=$foursq_client_secret&intent=browse&limit=10&v=20130611&ll=$city_lat,$city_lon&radius=$city_radius&query=$querystring";
		$full_url = "$places_url?key=$gmaps_api_key&location=$city_lat,$city_lon&radius=$city_radius&keyword=$querystring&types=establishment|route&sensor=true";
